...
import {getCondoList} from '../condos/api/condos.service';

export const startGetResidentHome = () => {
    return async (dispatch: any) => {
        return await getCondoList().then(resp => {
            if (resp.status === 202) {
                const {data} = resp;
...
export const startGetResident = () => {
    return async (dispatch) => {
        return await getResident().then(resp => {
            if (resp.status === 202) {
